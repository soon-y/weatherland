...
export function Windvane({ windDirDebug, windSpdDebug, gustsSpdDebug, windDir, windSpd, gustsSpd }) {
  ...
  useFrame((_, delta) => {
    ...
    time.current += delta
    gustTime.current += delta

    let targetDirection = windDirection
    let targetSpeed = windSpeed

    if (gustTime.current > 2 + Math.random() * 3) {
      gustTime.current = 0
      gustStrength.current = gustsSpeed
    }

    gustStrength.current *= 0.95

    direction.current += (targetDirection - direction.current) * 0.05
    speed.current += (targetSpeed - speed.current) * 0.05

    const sway = Math.sin(time.current * 2) * (speed.current * 0.001)
    const finalSpeed = speed.current + gustStrength.current

    arrow.rotation.y = direction.current + sway
    anemometer.rotation.y -= finalSpeed * 0.01
